...
def download_prices(tickers: Dict[str, str], start: dt.date, end: dt.date) -> pd.DataFrame:
    """Descarga y normaliza los precios ajustados para los tickers especificados."""
    ...
    # Cuando ``auto_adjust`` es ``False`` el precio ajustado se encuentra bajo la clave
    # ``Adj Close``. Sin embargo, en versiones recientes de ``yfinance`` el parámetro
    # cambió de valor por defecto y la columna puede no existir. Por lo tanto se
    # ofrece una ruta de respaldo utilizando ``Close`` (ya ajustado si ``auto_adjust``
    # es ``True``).
    if isinstance(raw_data.columns, pd.MultiIndex):
        price_key = "Adj Close" if "Adj Close" in raw_data.columns.levels[0] else "Close"
        prices = raw_data.xs(price_key, axis=1, level=0)
    else:
        price_key = "Adj Close" if "Adj Close" in raw_data.columns else "Close"
        prices = raw_data[price_key]

    prices = prices.rename(columns={ticker: name for name, ticker in tickers.items()})
